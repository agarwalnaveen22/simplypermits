...
function createTaskChainWithOutput({ output, colors = colors_1.DEFAULT_COLORS }) {
    const { failure, strong, success } = colors;
    const chain = new TaskChain({ taskOptions: { tickInterval: 50 } });
    chain.on('next', task => {
        task.on('success', () => {
            output.stream.write(`${success(exports.ICON_SUCCESS)} ${task.msg} - done!`);
        });
        task.on('failure', () => {
            output.stream.write(`${failure(exports.ICON_FAILURE)} ${task.msg} - failed!`);
        });
        if (guards_1.isRedrawLine(output)) {
...
exports.createTaskChainWithOutput = createTaskChainWithOutput;
