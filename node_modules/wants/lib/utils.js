...
var http = require('http'), crypto = require('crypto'), Path = require('path'), isWindows = process.platform === 'win32', // 是否是windows
fs = require('fs');

/**
 * Extract the mime type from the given request's _Content-Type_ header.
 * 
...
exports.sign = function(val, secret) {
	return val + '.' + crypto.createHmac('sha1', secret).update(val).digest('base64').replace(/=+$/, '');
};

/**
 * Unsign and decode the given `val` with `secret`, returning `false` if the
...
exports.unsign = function(val, secret) {
	var parts = val.split('.'), str = parts[0]
...
exports.parseSignedCookies = function(obj, secret) {
	...
	Object.keys(obj).forEach(function(key) {
		var val = obj[key], signed = exports.unsign(val, secret);
...
exports.parseCookie = function(str) {
	...
	for ( var i = 0, len = pairs.length; i < len; ++i) {
		var pair = pairs[i], eqlIndex = pair.indexOf('='), key = pair.substr(0, eqlIndex).trim(), val = pair.substr(++eqlIndex, pair.length).trim();
...
exports.modified = function(req, res) {
	var headers = res.headers || {}, modifiedSince = req.headers['if-modified-since'], lastModified = headers['last-modified'], noneMatch = req.headers['if-none-match'], etag = headers['etag'];
...
exports.errorRes = function(res, msg, scode) {
	...
	res.end(resMsg(500, msg, res._wantsReq));
}
exports.errorRes404 = function(res, msg) {
	var msg = msg || 'Error';
	res.statusCode = 404;
	res.end(resMsg(404, msg, res._wantsReq));
}
exports.forbidden = function(res, msg) { // 禁止访问函数
	var msg = msg || 'Forbidden';
	res.statusCode = 403;
	res.end(resMsg(403, msg, res._wantsReq));
};
exports.fdate = function(format, time) {// 格式化时间
	var format = typeof format === 'undefined' ? false : format.toLowerCase();
	var now = (time) ? time : (new Date());
	var _time = now.getFullYear() + "-" + (((now.getMonth() + 1) < 10) ? ("0" + (now.getMonth() + 1)) : (now.getMonth() + 1)) + "-"
			+ ((now.getDate() < 10) ? ("0" + now.getDate()) : now.getDate());
	if (format === 'y-m-d h:m:s') {
		_time += ' ' + now.getHours() + ":" + now.getMinutes() + ":" + now.getSeconds();
	}
	return _time;
};
...
var resMsg = exports.resMsg = function() {
	...
	return function(code, msg, req) {
		var str = '<html><head><title>' + code + ' ' + msg + '</title></head><body bgcolor="white"><center><h1>' + code + ' ' + msg
				+ '</h1></center><hr><center><p>wants node.js framework / ' + _wants_config.version
				+ ' <br/>flexable and high performace node.js restful framework</p></center></body></html>'
		str = headReq(req, str);
		return str;
	}
}();
exports.getVersion = function() {
	...
	_wants_config.version = version;
	return _wants_config.version;
}

exports.normalize_query = function(p) {
	var nor_path = Path.normalize(p);
	if (isWindows)
		return nor_path.replace(/\\/g, '/');
	return nor_path;
}

// check mongodb uid
exports.checkeuid = function(str) {
