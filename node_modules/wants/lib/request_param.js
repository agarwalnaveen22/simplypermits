...
var genparamdot = module.exports = function(reqparam) {
	var p = reqparam, keys = Object.keys(p), filter_keys = [], normal_keys = keys.filter(function(v, i) {
		if (~v.indexOf('.')) {
			filter_keys.push(v);
			return false
		}
	...
	if (keys.length === 0 || filter_keys.length === 0)
		return p;// 如果没有参数或者没有带"."的参数，则跳过

	var paramdot = {};// 存放paramdot的对象

	filter_keys.forEach(function(v, i, ary) {
		var v_ary = v.split('.'), v_last = v_ary.length - 1;
		var deal = function(obj, j, array) {
			var me = arguments.callee, islast = j === v_last ? true : false;
			if (islast)
				return obj[array[j]] = p[v];
			if (!obj[array[j]])
				obj[array[j]] = {};
